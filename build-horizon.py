...
options = ['-DBUILD_SHARED_LIBS=OFF', '-DPROFILE_SPIRV=OFF', '-DPROFILE_GLSPIRV=OFF', '-DCOMPILER_SUPPORT=OFF']
if 'MojoShader' == 'FNA3D':
...
run(['cmake', '--build', out, '--parallel', a.jobs, '--target', *['mojoshader']])
manifest = {'revision': subprocess.check_output(['git', '-C', ROOT, 'rev-parse', 'HEAD'], text=True).strip(),
            ...
            'archives': {name: digest(out / name) for name in ['libmojoshader.a']}}
if 'MojoShader' == 'SDL': manifest['archive_sha256'] = digest(out / 'libSDL2.a')
